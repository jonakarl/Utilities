...
if [ -d $_EXPPATH ]; then
  if [ -z "$IS_VM" ]; then 
    ...
    docker logs -t $CONTAINER_NAME &> $_EXPPATH/container.log;
    echo ""
  fi
  if [ ! -z "$STARTTIME" ]; then
...
#TODO_ SHould this be here or in pull-base-containers et al.
...
#TODO: Report with exist status if cleanup failed
umount $_EXPPATH
rmdir  $_EXPPATH
